...
deleteSupplier(supplier: Supplier): void {
  const confirmDelete = confirm("Do You Really Want To Delete Supplier");
  if (confirmDelete) {
    this.supplyLinkService.deleteSupplier(supplier.supplierId!).subscribe({
      next: () => {
        alert('Supplier deleted successfully.');

        // Check if the deleted supplier is the currently logged-in user
        const currentUserId = Number(localStorage.getItem("user_id"));
        const currentUserRole = localStorage.getItem("role");

        if (supplier.supplierId === currentUserId && currentUserRole === 'ADMIN') {
          // Clear local storage and redirect to /auth
          localStorage.clear();
          this.router.navigate(['/auth']);
        } else {
          // Reload data for other cases
          this.loadAdminData();
        }
      },
      error: (error) => {
